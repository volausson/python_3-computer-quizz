...
questions = [
    ...
    {
        'question': 'In computing there are 8 bits to a byte, '
        'but what are 4 bits called?',
        'options': [
           "a) A nibble",
           "b) A nipple",
           "c) A nizzle"
    ...
    {
        'question': 'What sort of animal is Tux, '
        'the official mascot of the Linux operating system?',
...
def start_quizz():
    ...
    while ask:
        question = input('Are you ready to play?\n'
                         'Please enter y for yes or '
                         'n for no:\n').lower().strip()
        if question == 'y':
            enter_quizz()
        if question == '':
            print('You need to type y for yes to play, '
                  'or n for no to exit game!\n')
        if question == 'n':
            print('~~~~~~~~~~~~~~~~~~~~~~')
...
def enter_quizz():
    ...
    player_name = input('Please enter your name, '
                        'otherwise I will just call you '
                        'Player:\n').lower().strip()
    if player_name == '':
        player_name = 'Player'
